...
{if ($field.type === 'radio-buttons')}radio_btn_group{/if}
field_{{$field.type}}_{{$field.name}}">
    ...
    <div class="col-md-6{if ($field.type === 'radio-buttons')} form-control-valign{/if}">
      ...
      {elseif $field.type === 'date' || $field.name === 'birthday'}
        ...
        {block name='form_field_item_date'}
          <input name="{$field.name}" class="form-control" type="date" pattern="[0-9]{2}-[0-9]{2}-[0-9]{4}" value="{$field.value}"
...
{*              placeholder="{l s='At least 5 characters long' d='Shop.Forms.Help'}"*}
              name="{$field.name}"
...
{*            placeholder="{$field.label}{if $field.required}*{/if}"*}
    ...
    <div class="col-md-3 form-control-comment">
      ...
      {if {{$field.name}} == 'password' || {{$field.name}} == 'birthday'}
          <div class="help_btn" data-popup="#help_field_{{$field.type}}_{{$field.name}}">?</div>
      {/if}
  </div>
